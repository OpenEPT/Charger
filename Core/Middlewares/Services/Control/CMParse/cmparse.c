...
cmparse_status_t 	CMPARSE_Execute(const char* command, char* response, uint16_t* responseSize)
{
	...
	{
		commandCallBack = prvCMPARSE_FindCallback(command + 1/*3*/);
	}
	...
	else
	{
		if(commandType == CMPARSE_COMMAND_TYPE_ASCII)
		{
			if(prvCMPARSE_GetArguments(command + 1/*3*/, arguments, &argumentsBufferSize) != CMPARSE_STATUS_OK) return CMPARSE_STATUS_ERROR;
		}
		if(commandType == CMPARSE_COMMAND_TYPE_BIN)
		{
